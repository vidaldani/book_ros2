...
namespace br2_fsm_bumpgo_cpp
{
  ...
  BumpGoNode::control_cycle()
  {
    ...
    case TURN:
    {
      //Checking what side has less obstacles
      size_t left_pos = last_scan_->ranges.size() / 4;
      size_t right_pos = last_scan_->ranges.size() / 4 * 3;
      if (last_scan_->ranges[left_pos] <= last_scan_->ranges[right_pos])
  ...
  BumpGoNode::check_forward_2_back()
  {
    ...
    return last_scan_->ranges[pos] < OBSTACLE_DISTANCE ||
            last_scan_->ranges[left_pos] < SIDE_DISTANCE ||
            last_scan_->ranges[right_pos] < SIDE_DISTANCE;
  }
